...
// 使用本地音頻文件或禁用外部資源以避免ORB錯誤
const CLICK_URL = null; // 'https://assets.mixkit.co/sfx/preview/mixkit-select-click-1109.mp3';
const SUCCESS_URL = null; // 'https://assets.mixkit.co/sfx/preview/mixkit-interface-1-1264.mp3';
const ERROR_URL = null; // 'https://assets.mixkit.co/sfx/preview/mixkit-system-beep-buzzer-fail-2964.mp3';
...
export const [SoundProvider, useSound] = createContextHook<SoundContextValue>(() => {
  ...
  const playWeb = useCallback(async (url: string | null) => {
    try {
      if (!url) return; // 如果 URL 為 null，直接返回
